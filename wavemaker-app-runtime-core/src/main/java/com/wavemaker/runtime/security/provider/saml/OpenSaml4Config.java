...
public class OpenSaml4Config {
    ...
    public OpenSaml4AuthenticationProvider samlAuthenticationProvider() {
        ...
        if (roleMappingEnabled && StringUtils.isNotBlank(roleProvider)) {
            if (roleProvider.equalsIgnoreCase("SAML") && StringUtils.isNotBlank(roleAttributeName)) {
                openSaml4AuthenticationProvider.setResponseAuthenticationConverter(samlAttributesBasedAuthenticationConverter());
            } else if (roleProvider.equalsIgnoreCase("DATABASE")) {
                openSaml4AuthenticationProvider.setResponseAuthenticationConverter(samlDatabaseBasedAuthenticationConverter());
            }
        }
        return openSaml4AuthenticationProvider;
    }

    private Converter<OpenSaml4AuthenticationProvider.ResponseToken, ? extends AbstractAuthenticationToken> samlDatabaseBasedAuthenticationConverter() {
        return (responseToken) -> {
            ...
            if (principal instanceof Saml2AuthenticatedPrincipal) {
                ...
                DefaultSaml2AuthenticatedPrincipal newSaml2Principal = new DefaultSaml2AuthenticatedPrincipal(username, saml2Principal.getAttributes());
    ...
    private Converter<OpenSaml4AuthenticationProvider.ResponseToken, ? extends AbstractAuthenticationToken> samlAttributesBasedAuthenticationConverter() {
        return responseToken -> {
            Saml2Authentication saml2Authentication = OpenSaml4AuthenticationProvider.createDefaultResponseAuthenticationConverter().convert(responseToken);
            Object principal = saml2Authentication.getPrincipal();
            if (principal instanceof Saml2AuthenticatedPrincipal) {
                ...
                DefaultSaml2AuthenticatedPrincipal newSaml2AuthenticatedPrincipal = new DefaultSaml2AuthenticatedPrincipal(saml2Principal.getName(), saml2Principal.getAttributes());
                newSaml2AuthenticatedPrincipal.setRelyingPartyRegistrationId(saml2Principal.getRelyingPartyRegistrationId());
                return new Saml2Authentication(newSaml2AuthenticatedPrincipal,
                    saml2Authentication.getSaml2Response(),
                    grantedAuthorities);
            }
            throw new WMRuntimeException("Error while creating Saml2Authentication, principal is not of type Saml2AuthenticatedPrincipal");
