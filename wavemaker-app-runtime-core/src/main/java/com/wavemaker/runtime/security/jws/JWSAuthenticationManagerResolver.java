...
public class JWSAuthenticationManagerResolver implements AuthenticationManagerResolver<String> {
    ...
    @Value("${security.providers.jws.activeProviders}")
    private String jwsActiveProviders;
    ...
    @PostConstruct
    private void init() {
        List<String> jwsProviders = Arrays.stream(jwsActiveProviders.split(",")).collect(Collectors.toList());
        jwsProviders.forEach(jwsProviderId -> {
        ...
        if (this.trustedIssuer.test(issuer)) {
            return this.authenticationManagers.computeIfAbsent(issuer,
                (k) -> {
                    ...
                    if (issuerUrlVsProviderId.containsKey(issuer)) {
                        String jwsProviderId = issuerUrlVsProviderId.get(issuer);
                        String principalClaimName = environment.getProperty("security.providers.jws." + jwsProviderId + ".principalClaimName", "sub");
